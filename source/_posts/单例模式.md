...
2. 让线程也拥有资源，不用去共享进程中的资源。如：threadlocal为每个线程维护一个私有的本地变量。
...
```
运行结果如下。从运行结果可以看出，单例构造函数创建了两个，内存地址分别为**0x7f3c980008c0**和**0x7f3c900008c0**。所以普通懒汉式单例只适合单进程不适合多进程，因为线程是不安全的。
![普通懒汉式单例运行结果](./images/单例模式/懒汉式单例运行结果.png)
...
///////////////////  加锁的懒汉式实现  //////////////////
```
运行结果如下图。从运行结果可以看出，程序只创建了一个实例，内存地址是**0x7f28b00008c0**，所以加了互斥锁的普通懒汉式是线程安全的。
![加锁的懒汉式单例运行结果](./images/单例模式/加锁的懒汉式单例运行结果.png)
...
![内部静态变量的懒汉单例运行结果](./images/单例模式/内部静态变量的懒汉单例运行结果.png)
...
![饿汉式单例运行结果](./images/单例模式/饿汉式单例运行结果.png)
