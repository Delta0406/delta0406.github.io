...
![_heap_init()和_sbh_heap_init()](内存管理_100.jpg)
![HEADER结构](内存管理_101.jpg)
...
![_heap_alloc_dbg()1](内存管理_103.jpg)
![_heap_alloc_dbg()2](内存管理_104.jpg)
...
![_heap_alloc_base()](内存管理_105.jpg)

### __sbh_alloc_block()
到这个阶段，才开始为内存块加上`cookie`，并通过`BYTES_PER_PARA`做16字节的对齐。

![__sbh_alloc_block()](内存管理_106.jpg)
...
![__sbh_alloc_new_region](内存管理_107.jpg)
...
![__sbh_alloc_new_group()](内存管理_108.jpg)

当前阶段是初始化阶段，8个`page`是直接由`group`链表中的最后一组来管理的，这组链表用来管理超过`1k`的内存。
![__sbh_alloc_new_group()_page](内存管理_109.jpg)
...
![SBH行为分析——第2次，分配](内存管理_112.jpg)
![SBH行为分析——第3次，分配](内存管理_113.jpg)
