...
![VC6.0 标准分配器实现](vc6_allocator_implement.png)
...
## bc5 allocator
![bc5 标准分配器实现](bc5_malloc_implement.png)

## G2.9 allocator
![G2.9 标准分配器实现](g2_9_malloc_implement.png)

G2.9版本的allocator实现如上图所示，但其并没有实际使用。实际上，G2.9是使用了一个`alloc`类实现内存管理，该类分配内存以**字节**为单位，而非对象。如下图所示：

![G2.9 alloc](g2_9_alloc.png)

## GCC4.9 allocator
GCC4.9版本将GCC2.9里不属于正式使用的版本`std::alloc`转变成`_pool_alloc`，修改了变量名和部分操作。
![std::alloc vs _pool_alloc1](pool_alloc_1.png)

![std::alloc vs _pool_alloc2](pool_alloc_2.png)

而GCC4.9的标准分配器实现仍是以`::operator new`和`::operator delete`实现，没有特殊设计。

![G4.9 标准分配器实现](g4_9_standard_implement1.png)

![g4.9 pool allocator 用例](g4_9_pool_allocator.png)
...
![std::alloc运行模式](std_alloc运行模式总览.png)
...
![embedded pointers](embedded_pointers.png)
...
## `std::alloc`运行一瞥
![std::alloc运行一瞥01](std_alloc_01.png)

链表上方小块表示cookie。
![std::alloc运行一瞥02](std_alloc02.png)
![std::alloc运行一瞥03](std_alloc03.png)

`RoundUp`是追加量，值为目前累计申请量除以16，以适应越来越高的内存要求。
![std::alloc运行一瞥04](std_alloc04.png)

申请内存时，首先看战备池是否有足够的内存，如果有则直接从池中分配。
![std::alloc运行一瞥05](std_alloc05.png)
![std::alloc运行一瞥06](std_alloc06.png)
![std::alloc运行一瞥07](std_alloc07.png)
![std::alloc运行一瞥08](std_alloc08.png)
![std::alloc运行一瞥09](std_alloc09.png)
![std::alloc运行一瞥10](std_alloc10.png)
![std::alloc运行一瞥11](std_alloc11.png)
![std::alloc运行一瞥12](std_alloc12.png)
![std::alloc运行一瞥13_01](std_alloc13_01.png)
![std::alloc运行一瞥13_02](std_alloc13_02.png)
...
![std::alloc源码剖析4](std_alloc_源码剖析4.png)
...
![std::alloc源码剖析5](std_alloc_源码剖析5.png)
![std::alloc源码剖析5_02](std_alloc_源码剖析5_02.png)

释放内存则需要先判断待释放的内存空间是否属于二级分配器管理，不属于则调用一级分配器处理。属于则拿到待释放内存对应的空闲链表，将待释放内存块的`free_list_link`指向拿到的空闲链表表头，并设置空闲链表表头为待释放内存块。
![std::alloc源码剖析5_03](std_alloc_源码剖析5_03.png)
...
![std::alloc源码剖析8](std_alloc_源码剖析_8.png)
...
![std::alloc源码剖析6](std_alloc_源码剖析6.png)
![std::alloc源码剖析7](std_alloc_源码剖析7.png)

#### 变量初始化
![std::alloc源码剖析9](std_alloc_源码剖析9.png)
...
![std::alloc源码剖析1](std_alloc_源码剖析1.png)
...
![std::alloc观念整理1](std_alloc观念整理1.png)
